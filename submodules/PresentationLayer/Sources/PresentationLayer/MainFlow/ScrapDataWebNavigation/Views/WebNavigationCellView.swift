...
class WebNavigationCellView: UICollectionViewCell {
	...
	func configure(with uiModel: WebListCellUiModel) {
		self.label.text = uiModel.title
		self.imageView.image = uiModel.image
		self.tickImage.isHidden = !uiModel.isChecked
	}
}
